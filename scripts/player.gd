...
func setup(arena_rect: Rect2, start_position: Vector2, goal_y_threshold: float) -> void:
...
func request_move(direction: Vector2i) -> void:
	...
	var target: Vector2 = global_position + Vector2(direction) * tile_size
	target = _clamp_to_arena(target)
	if target.is_equal_approx(global_position):
		return

	_handle_row_advance(target)
	_start_tween(target)
	_check_goal(target)
...
func _handle_row_advance(target: Vector2) -> void:
	var rows_up: int = int(round((_start_row_y - target.y) / tile_size))
	if rows_up > _max_row_reached:
		var rows_gained: int = rows_up - _max_row_reached
		_max_row_reached = rows_up
...
func _check_goal(target: Vector2) -> void:
